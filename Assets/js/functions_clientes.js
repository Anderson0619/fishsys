...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if(document.querySelector("#formCliente")){
        ...
        formCliente.onsubmit = function(e) {
            ...
            let intTipoCliente = document.querySelector('#listTipClient').value;
            let strDireccion = document.querySelector('#txtDireccion').value;
            if(strIdentificacion == '' || strApellido == '' || strNombre == '' || strEmail == '' || intTelefono == '' || intTipoCliente == '' || strDireccion == '' )
            {
    ...
    request.onreadystatechange = function(){
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                var estadoTipoCliente = objData.data.status == 1 ? 
                '<span class="badge">COMERCIANTE</span>' : 
                '<span class="badge badge-success">EMPRESA</span>';
                '<span class="badge badge-success">EMPLEADOS</span>';
                ...
                document.querySelector("#celTipCliente").innerHTML = estadoTipoCliente;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                //document.querySelector("#listTipClient").value =objData.data.tipocliente;
                document.querySelector("#txtDireccion").value =objData.data.direccion;
                //$('#listTipClient').selectpicker('render');

                if (objData.data.status == 1) {
                    document.querySelector("#listTipClient").value = 1;
                }else if (objData.data.status == 2){
                    document.querySelector("#listTipClient").value = 2;
                } else if(objData.data.status == 3){
                    document.querySelector("#listTipClient").value = 3;
                }else{
                    document.querySelector("#listTipClient").value = 4;
                }
                $('#listTipClient').selectpicker('render');
            }
        }
